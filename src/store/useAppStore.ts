...
import type { Note, Folder, Task, Subtask, ViewMode, ActiveView, Board, BoardColumn, BoardTask, BoardComment, LinkedItem, Attachment } from '../types'
import { extractTitleFromContent, setContentTitle, slugifyTitle } from '../lib/utils'
...
import { isDarkTheme, loadCustomThemes, persistCustomThemes, type CustomTheme } from '../lib/themes'
...
interface AppState {
  ...
  setEditorSettings: (settings: Partial<{ editorFontSize: string; editorFontFamily: string; editorLineHeight: string }>) => void
  ...
  closeConfirm: () => void
  selectNote: (id: string, options?: { additive?: boolean; range?: boolean; orderedIds?: string[] }) => void
  ...
  reorderNote: (noteId: string, targetFolderId: string | null, insertBeforeNoteId: string | null) => void
  moveFolder: (folderId: string, targetParentId: string | null, insertBeforeFolderId: string | null) => void
  ...
  createBoardTask: (columnId: string, title: string, priority?: BoardTask['priority']) => void
  updateBoardTask: (taskId: string, updates: Partial<Omit<BoardTask, 'id' | 'boardId'>>) => void
  deleteBoardTask: (taskId: string) => void
  moveBoardTask: (taskId: string, toColumnId: string, beforeTaskId?: string | null) => void
...
function updateFolderNotes(folders: Folder[], noteId: string, updater: (n: Note) => Note): Folder[] {
...
function insertFolderIntoTree(folders: Folder[], parentId: string | null, newFolder: Folder): Folder[] {
  if (parentId === null) return [...folders, newFolder]
  return folders.map(f => {
    if (f.id === parentId) {
      return { ...f, expanded: true, children: [...f.children, newFolder] }
    }
    return { ...f, children: insertFolderIntoTree(f.children, parentId, newFolder) }
  })
}

function addNoteToFolderTree(folders: Folder[], folderId: string, note: Note): Folder[] {
  return folders.map(f => {
    if (f.id === folderId) {
      return { ...f, notes: [note, ...f.notes.filter(n => n.id !== note.id)] }
    }
...
function insertNoteIntoFolderAt(folders: Folder[], folderId: string, note: Note, insertBeforeId: string | null): Folder[] {
  return folders.map(f => {
    if (f.id === folderId) {
      ...
      return { ...f, notes: idx === -1 ? [...rest, note] : [...rest.slice(0, idx), note, ...rest.slice(idx)] }
    }
    return { ...f, children: insertNoteIntoFolderAt(f.children, folderId, note, insertBeforeId) }
  })
}

function insertFolderAt(folders: Folder[], parentId: string | null, folder: Folder, insertBeforeId: string | null): Folder[] {
  if (parentId === null) {
    ...
    return idx === -1 ? [...rest, folder] : [...rest.slice(0, idx), folder, ...rest.slice(idx)]
  }
  return folders.map(f => {
    if (f.id === parentId) {
      const rest = f.children.filter(c => c.id !== folder.id)
      if (insertBeforeId === null) return { ...f, children: [...rest, folder], expanded: true }
      const idx = rest.findIndex(c => c.id === insertBeforeId)
      return {
        ...f, expanded: true,
        children: idx === -1 ? [...rest, folder] : [...rest.slice(0, idx), folder, ...rest.slice(idx)],
      }
    }
    return { ...f, children: insertFolderAt(f.children, parentId, folder, insertBeforeId) }
  })
}

function buildFolderPath(folders: Folder[], folderId: string): string {
  const folder = findFolderById(folders, folderId)
  if (!folder) return '~/Documents/Inkwell'
  if (folder.parentId) {
    const parentPath = buildFolderPath(folders, folder.parentId)
    return `${parentPath}/${folder.name}`
  }
  return `~/Documents/Inkwell/${folder.name}`
}
...
function removeNotesFromFolderTree(folders: Folder[], noteIds: Set<string>): Folder[] {
  return folders.map(f => ({
    ...f,
...
function findFolderPath(folders: Folder[], id: string, path: string[] = []): string[] | null {
  for (const f of folders) {
...
function isFolderInsideDeleted(folders: Folder[], deletedId: string, folderId: string | null): boolean {
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  editorFontFamily: localStorage.getItem('inkwell-editor-fontFamily') ?? "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
  ...
  openVault: (path, data) => {
    ...
    let notes = (data?.notes ?? []).map(n => ({ ...n, linkedItems: n.linkedItems ?? [] }))
  ...
  clearNoteSelection: () => set({ selectedNoteIds: [], lastSelectedNoteId: null }),
  clearFolderSelection: () => set({ selectedFolderId: null, selectedNoteIds: [], lastSelectedNoteId: null }),
  ...
  toggleFolder: (id) => set(s => ({ folders: toggleFolderById(s.folders, id) })),
  ...
  setEditorSettings: (settings) => {
    if (settings.editorFontSize) localStorage.setItem('inkwell-editor-fontSize', settings.editorFontSize)
    if (settings.editorFontFamily) localStorage.setItem('inkwell-editor-fontFamily', settings.editorFontFamily)
    if (settings.editorLineHeight) localStorage.setItem('inkwell-editor-lineHeight', settings.editorLineHeight)
    set(settings)
  },
  ...
  pinNote: (id) => {
    set(s => ({
      ...
      folders: updateFolderNotes(s.folders, id, n => ({ ...n, pinned: !n.pinned })),
    }))
  },
  ...
  createFolder: (name, parentId = null) => {
    ...
    const parent = parentId ? findFolderById(get().folders, parentId) : null
    const path = parent
      ? `${parent.path}/${trimmed}`
      : `~/Documents/Inkwell/${trimmed}`
  ...
  createNote: (folderId) => {
    ...
    const folderPath = folderId
      ? buildFolderPath(get().folders, folderId)
      : '~/Documents/Inkwell'
    const newNote: Note = {
      id,
      title: 'Untitled',
      content: '',
      path: `${folderPath}/untitled-${Date.now()}.md`,
      folder: folderId,
      tags: [],
      pinned: false,
      createdAt: new Date(),
  ...
  moveNote: (noteId, targetFolderId) => {
    ...
    const folderPath = targetFolderId
      ? buildFolderPath(get().folders, targetFolderId)
      : '~/Documents/Inkwell'
  ...
  reorderNote: (noteId, targetFolderId, insertBeforeNoteId) => {
    ...
    if (note.folder === targetFolderId) {
      ...
      set(s => {
        if (targetFolderId === null) {
          ...
          return { notes: idx === -1 ? [...rest, note] : [...rest.slice(0, idx), note, ...rest.slice(idx)] }
        }
        return {
          folders: insertNoteIntoFolderAt(
            removeNoteFromFolderTree(s.folders, noteId),
            targetFolderId, note, insertBeforeNoteId,
          ),
        }
      })
      return
    ...
    const folderPath = targetFolderId ? buildFolderPath(folders, targetFolderId) : '~/Documents/Inkwell'
    const filename = note.path.split('/').pop()!
    const updated = { ...note, folder: targetFolderId, path: `${folderPath}/${filename}`, updatedAt: new Date() }

    set(s => {
      let foldersNew = removeNoteFromFolderTree(s.folders, noteId)
      let notesNew = s.notes.map(n => n.id === noteId ? updated : n)
      if (targetFolderId) {
        foldersNew = insertNoteIntoFolderAt(foldersNew, targetFolderId, updated, insertBeforeNoteId)
      } else {
        notesNew = s.notes.filter(n => n.id !== noteId)
        const idx = insertBeforeNoteId ? notesNew.findIndex(n => n.id === insertBeforeNoteId) : -1
        notesNew = idx === -1 ? [...notesNew, updated] : [...notesNew.slice(0, idx), updated, ...notesNew.slice(idx)]
      }
  ...
  moveFolder: (folderId, targetParentId, insertBeforeFolderId) => {
    ...
    const { folders } = get()
    if (targetParentId && isFolderInsideDeleted(folders, folderId, targetParentId)) return
    const folder = findFolderById(folders, folderId)
    if (!folder) return
    const updated = { ...folder, parentId: targetParentId }
    set(s => ({
      folders: insertFolderAt(removeFolderFromTree(s.folders, folderId), targetParentId, updated, insertBeforeFolderId),
    }))
  },
  ...
  updateNote: (id, content) => {
    ...
    set(s => ({
      notes: s.notes.map(n =>
        n.id === id ? { ...n, content, title, wordCount, updatedAt: new Date() } : n
      ),
      folders: updateFolderNotes(s.folders, id, n => ({
        ...n, content, title, wordCount, updatedAt: new Date(),
      })),
    }))
  },
  ...
  renameNote: (id, title) => {
    ...
    const noteFolderId = note.folder
    const folderPath = noteFolderId
      ? buildFolderPath(get().folders, noteFolderId)
      : '~/Documents/Inkwell'
    const path = `${folderPath}/${slugifyTitle(trimmed)}.md`

    set(s => ({
      notes: s.notes.map(n =>
        n.id === id ? { ...n, title: trimmed, content, path, updatedAt: new Date() } : n
      ),
      folders: updateFolderNotes(s.folders, id, n => ({
        ...n, title: trimmed, content, path, updatedAt: new Date(),
      })),
    }))
  },
  ...
  deleteNotes: (ids) => {
    ...
    set(s => {
      ...
      if (lastSelectedNoteId && idSet.has(lastSelectedNoteId)) {
        lastSelectedNoteId = selectedNoteIds[selectedNoteIds.length - 1] ?? null
      }
      ...
      return { folders, notes, selectedNoteIds, lastSelectedNoteId, selectedFolderId }
    })
  },
  ...
  deleteFolder: (id) => {
    ...
    set(s => {
      ...
      let selectedNoteIds = s.selectedNoteIds.filter(id => !noteIdsToDelete.has(id))
      let lastSelectedNoteId = s.lastSelectedNoteId
      let selectedFolderId = s.selectedFolderId
  ...
  addNoteLink: (noteId, item) => {
    const updateNote = (n: Note) => {
      ...
      const already = (n.linkedItems ?? []).some(l => l.id === item.id && l.type === item.type)
      if (already) return n
      return { ...n, linkedItems: [...(n.linkedItems ?? []), item] }
    }
    set(s => ({
      notes: s.notes.map(updateNote),
  ...
  removeNoteLink: (noteId, itemId) => {
    const updateNote = (n: Note) =>
      n.id !== noteId ? n : { ...n, linkedItems: (n.linkedItems ?? []).filter(l => l.id !== itemId) }
    set(s => ({
      notes: s.notes.map(updateNote),
      folders: updateFolderNotes(s.folders, noteId, updateNote),
    }))
  ...
  addAttachment: (noteId, attachment) => {
    const updateNote = (n: Note) =>
      n.id !== noteId ? n : { ...n, attachments: [...(n.attachments ?? []), attachment] }
    set(s => ({
      notes: s.notes.map(updateNote),
      folders: updateFolderNotes(s.folders, noteId, updateNote),
    }))
  ...
  removeAttachment: (noteId, attachmentId) => {
    const updateNote = (n: Note) =>
      n.id !== noteId ? n : { ...n, attachments: (n.attachments ?? []).filter(a => a.id !== attachmentId) }
    set(s => ({
      notes: s.notes.map(updateNote),
      folders: updateFolderNotes(s.folders, noteId, updateNote),
    }))
  ...
  createBoard: (name) => {
    ...
    const board: Board = { id: boardId, name, columnIds: columns.map(c => c.id) }
    set(s => ({
  ...
  addBoardColumn: (boardId, name) => {
    ...
    const column: BoardColumn = { id: colId, boardId, name, color: 'blue', taskIds: [] }
    set(s => ({
  ...
  renameBoardColumn: (columnId, name) => {
    set(s => ({
      boardColumns: s.boardColumns.map(c => c.id === columnId ? { ...c, name } : c),
    }))
  },
    ...
    set(s => {
      ...
      if (updates.columnId && updates.columnId !== task.columnId) {
        const columns = s.boardColumns.map(c => {
          if (c.id === task.columnId) return { ...c, taskIds: c.taskIds.filter(id => id !== taskId) }
          if (c.id === updates.columnId) return { ...c, taskIds: [...c.taskIds, taskId] }
          return c
        })
        return {
      ...
      return { boardTasks: s.boardTasks.map(t => t.id === taskId ? newTask : t) }
    })
  },
  ...
  deleteBoardTask: (taskId) => {
    set(s => {
      ...
      return {
        ...
        boardColumns: s.boardColumns.map(c =>
          c.id === task.columnId ? { ...c, taskIds: c.taskIds.filter(id => id !== taskId) } : c
        ),
      }
    })
  },
    ...
    set(s => {
      ...
      const columns = s.boardColumns.map(c => {
        ...
        if (c.id === toColumnId) {
          ...
          if (beforeTaskId === null) return { ...c, taskIds: [...filtered, taskId] }
      ...
      return {
        boardColumns: columns,
        boardTasks: s.boardTasks.map(t => t.id === taskId ? { ...t, columnId: toColumnId } : t),
      }
    })
  },
}))
